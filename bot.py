...
class Database:
    def __init__(self, uri, name):
        ...
        # Fuzzy search uses distinct movie titles rather than every media version.
        # Cache that title list briefly so typo correction stays fast under normal
        # repeated Request Group traffic.
        self._title_cache = None
        self._title_cache_keys = set()
        self._title_cache_at = 0.0
        self._title_cache_ttl = 60.0
        self._title_cache_lock = asyncio.Lock()
    ...
    async def upsert_movie(self, doc):
        ...
        if title and self._title_cache is not None:
            key = normalize(title)
            if key and key not in self._title_cache_keys:
                self._title_cache.append(title)
                self._title_cache_keys.add(key)

    def _movie_search_criteria(self, query, filters=None):
        filters = filters or {}
        words = [w for w in normalize(query).split() if len(w) >= 2]
        if not words:
            return None

        # Each search word must occur somewhere in the combined normalized
        # title + caption + filename field. Order is intentionally irrelevant.
        # This means a phrase such as "Hindi Naruto" still matches a caption
        # containing "Naruto ... Hindi ...".
        criteria = {
            "$and": [
                {"normalized": {"$regex": re.escape(word), "$options": "i"}}
                for word in words
            ]
        }
    ...
    async def count_movies(self, query, filters=None):
        criteria = self._movie_search_criteria(query, filters)
        if criteria is None:
            return 0
        return int(await self.movies.count_documents(criteria))
    ...
    async def find_movies(self, query, limit, filters=None, skip=0):
        ...
        cursor = (
            ...
            .sort([("quality_rank", ASCENDING), ("title", ASCENDING), ("message_id", ASCENDING)])
            .skip(skip)
            .limit(limit)
        )
    ...
    async def _get_title_candidates(self):
        ...
        if self._title_cache is not None and now - self._title_cache_at < self._title_cache_ttl:
            return self._title_cache

        async with self._title_cache_lock:
            now = asyncio.get_running_loop().time()
            if self._title_cache is not None and now - self._title_cache_at < self._title_cache_ttl:
                return self._title_cache
            try:
                titles = await self.movies.distinct(
                ...
                self._title_cache = cleaned
                self._title_cache_keys = {normalize(title) for title in cleaned}
                self._title_cache_at = now
            except Exception:
                log.exception("Could not collect distinct titles for fuzzy search")
                return self._title_cache or []
        return self._title_cache or []
    ...
    async def suggestions(self, query, limit=5):
        ...
        # Score distinct titles instead of thousands of duplicate file versions.
        # The title list is cached briefly so repeated typo searches stay fast.
        titles = await self._get_title_candidates()

        scored = []
        for title in titles:
            score = self._fuzzy_title_score(q, title)
            # A high-confidence single correction is our main UX target.
            # Lower-scoring candidates are allowed so a small group of genuinely
            # close titles can still be shown to the user.
            if score >= 0.58:
                scored.append((score, title))

        scored.sort(key=lambda item: (-item[0], item[1].lower()))
        return [title for score, title in scored[: max(1, limit)]]
...
def compact_caption_preview(movie, max_chars=180):
    ...
    """
    caption = str(movie.get("caption") or "").strip()
    if not caption:
        return ""
...
def search_header(query,total_results,page,pages,filters_,page_size=8):
    ...
    return ("👑 <b>CINEMA HUB OG</b>\n<i>PREMIUM MOVIE SEARCH</i>\n\n"
            ...
            f"✅ <b>{total_results} result(s)</b> • <b>{shown_start}-{shown_end}</b> shown\n"
            + ("🎛 <b>"+" • ".join(active)+"</b>\n" if active else "")
...
async def render_search_message(
    ...
    # Results passed to the renderer are already the current page. The batch button
    # therefore remains fast and does not force us to load the full result set.
...
async def begin_gate(update, context, movie_ids):
    ...
    await send_linkpays(update, context, movie_ids)
...
async def fsub_check(update, context):
    ...
    await send_linkpays(query,context,gate["movie_ids"])

async def send_linkpays(update, context, movie_ids):
    cfg = context.application.bot_data["cfg"]
    db = context.application.bot_data["db"]
    user_id = update.effective_user.id
...
async def group_search(update, context):
    ...
    try:
        total_results, results = await asyncio.gather(
            db.count_movies(query),
            db.find_movies(query, cfg.page_size),
        )
    except Exception:
    ...
    if not total_results:
        ...
        if suggestions:
            ...
            buttons=[]
            for index, title in enumerate(suggestions[:3]):
                buttons.append([
                    InlineKeyboardButton(
                        f"🎬 {title[:48]}",
                        callback_data=f"suggest:{sid}:{index}",
                    )
                ])
            suggestion_text=(
                "👑 <b>CINEMA HUB OG</b>\n"
                "<i>SMART TITLE CORRECTION</i>\n\n"
                "🔎 <b>DID YOU MEAN?</b>\n\n"
                "Tap the closest title below and I’ll continue automatically."
            )
            try:
                ...
                if "message is not modified" not in str(exc).lower():
                    sent = await searching.reply_text(
                        suggestion_text,
                        reply_markup=InlineKeyboardMarkup(buttons),
                        parse_mode="HTML",
                    )
                    await _persist_search_result_message(sent, db, sid, update.effective_user.id, cfg)
        else:
            sid=await db.create_search(
                update.effective_user.id,
                query,
            ...
            no_match_text=(
                ...
                f"<blockquote>{escape(query)}</blockquote>\n"
                "Please request this title in the group."
            )
...
async def filter_callback(update, context):
    ...
    cfg=context.application.bot_data["cfg"]
    total_results=await db.count_movies(doc["query"],filters_)
    if not total_results: await q.answer("No results for this filter.",show_alert=True); return
    results=await db.find_movies(doc["query"],cfg.page_size,filters_)
    await q.answer("Filter applied ✅"); await render_search_message(q.message,context,results,search_id,0,doc["query"],q.from_user.id,total_results)
